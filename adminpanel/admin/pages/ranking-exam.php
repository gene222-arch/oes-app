                    ...
                    <div class="page-title-wrapper">
                        <div class="page-title-heading">
                            <div><b class="text-primary">RANKING BY EXAM</b><br>
                                Exam Name : <?= $selEx['ex_title'] ?><br><br>
                               ...
                               <span class="border" style="padding:10px;color:black;background-color: grey;">Not Answering</span>
                            </div>
                        </div>
                    </div>
                    </div>
                          ...
                          <tbody>
                            ...
                            <?php 
								...
								{ 
									...
									$selScoreQuery = "SELECT 
											* 
										FROM 
											exam_question_tbl eqt 
										INNER JOIN 
											exam_answers ea 
										ON 
											eqt.eqt_id = ea.quest_id 
										AND 
											eqt.exam_answer = ea.exans_answer 
										WHERE 
											ea.axmne_id = $exmneId 
										AND 
											ea.exam_id = $exam_id 
								...
								$selAttempt = $conn->query($selAttemptQuery);
								$over = $selEx['ex_questlimit_display'];    

								$ans = ($selScore->rowCount() / $over) * 100;

								if (! $selAttempt->rowCount()) {
									$trStyle = "background-color: #E9ECEE;color:black";
								} else if ($ans >= 90) {
									$trStyle = "background-color: yellow;";
								}  else if ($ans >= 80) {
									$trStyle = "background-color: green;color:white";
								} else if ($ans >= 75) {
									$trStyle = "background-color: blue;color:white";
								} else {
									$trStyle = "background-color: red;color:white";
								}
							?>
